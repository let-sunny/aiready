...
| 2 | Find install instruction | Scroll to the `## Installation` section of the README. Note which install command appears **first**. | The first command a fresh user encounters for the design-to-code story is `npx canicode init` (interactive prompt) or `npx canicode init --token figd_xxx` (CI / non-TTY) — the one that installs the skills. If a different command (e.g. `npx canicode analyze`) appears first and leads the reader down the analyze-only path, flag as ⚠️ and record what they see. | | |
| 3 | Run install | In your fresh empty directory, run **either** `npx canicode init` (no flags — should prompt `Figma token:` and accept your token) **or** `npx canicode init --token figd_xxxxxxxxxxxxx`. Both branches must produce the same downstream output. | Stdout includes `Config saved:` (with a path), `Reports will be saved to:` (with a path), `Skills installed to: …/.claude/skills/`, and three counter lines (`installed:`, `overwritten:`, `skipped:`); on a fresh install the `installed` count equals the number of bundled skill files (currently 4 — three `SKILL.md` files plus `canicode-roundtrip/helpers.js`) and `overwritten` / `skipped` are both 0. Afterwards, `ls .claude/skills/` lists **three** directories (`canicode`, `canicode-gotchas`, `canicode-roundtrip`), and `ls .claude/skills/canicode-roundtrip/` lists **both** `SKILL.md` and `helpers.js`. Afterwards the command prints a `Next:` checklist: (1) `claude mcp add -s project -t http figma https://mcp.figma.com/mcp`, (2) `Restart Claude Code (so the new skills + Figma MCP tools both load)`, (3) `Run /canicode-roundtrip <figma-url>`. If the fresh directory already has a `.mcp.json` registering a `figma` entry, step (1) is omitted and step (2) mentions skills only. Mark ⚠️ if the checklist or the Figma MCP install command is missing, not if the 2-step variant appears. | | |
| 3b | Rotate the saved token | Run `canicode config set-token --token figd_yyyyyyyyyyyy` (or `canicode config set-token` interactively in a TTY). Then run `canicode config show`. | `set-token` prints `Token saved: …/canicode/config.json` and does **not** re-copy any skill files (no `Skills installed to:` block). `config show` prints the masked rotated token (e.g. `figd_••••••••yyyy`) plus config + reports paths. Mark ⚠️ if `set-token` re-runs the skill install or if the masked token is not displayed. (#505) | | |
...
| 5b | Code Connect prereq soft-warn (Step 1.5, #515) | In your fresh empty directory, you do **not** have `@figma/code-connect` or `figma.config.json` yet. After Step 1 (analyze) finishes, the roundtrip should print the doctor's remediation lines plus a `Continue anyway? (Y/n)` prompt. Type `Y` (or just Enter for the default) to continue. | The skill prints the same `❌ @figma/code-connect not installed` / `❌ figma.config.json not found` lines you would see from `canicode doctor` directly, followed by one preface line beginning with `"Code Connect is not configured in this repo."` and a `(Y/n)` prompt that defaults to `Y`. Mark ⚠️ if the soft-warn does not appear at all (means Step 1.5 was skipped), or if the prompt's default is not `Y`. Mark ❌ if accepting `Y` aborts the roundtrip instead of continuing to Step 2. | | |
...
| 9 | Handoff to `figma-implement-design` | Read how the skill tells you to generate code. | **Per ADR-013**, canicode's scope ends at Figma augmentation — code generation belongs to Figma's official `figma-implement-design` skill. The roundtrip skill should hand you off to `figma-implement-design` explicitly, not attempt to package code generation itself. If the skill ends at "ready for code generation" without naming the downstream skill, flag as ⚠️ — a fresh user won't know what to invoke next. After the handoff completes, the skill should proceed to **Step 7** (Code Connect close-out, #515) — see row 9b. | | |
| 9b | Code Connect close-out (Step 7, #515) | After `figma-implement-design` returns, watch what the roundtrip skill does next. Because you skipped the prereq install in row 5b, the skill should hit `canicode doctor` again and short-circuit. | Step 7a re-runs `canicode doctor` and again sees both checks fail. The skill prints the doctor's remediation followed by exactly one closing line stating that **steps 1–6 succeeded** and the **mapping was skipped** because Code Connect prerequisites are missing. The wrap-up rubric ends with `Code Connect: ⏭️ skipped (prereq)`. The earlier roundtrip outcome (issues delta, generated code path) is still reported — the failure of Step 7 must not retroactively invalidate the earlier steps. Mark ⚠️ if the skill prints the satisfaction prompt anyway (means Step 7a was skipped), or if no closing wrap-up line appears at all. To exercise the **happy path** (mapping actually registered): install `@figma/code-connect` and add a minimal `figma.config.json` in the test directory before invoking, then expect Step 7b to ask `Are you satisfied with this implementation? (y/N)` defaulting to `N`, and on `y` the wrap-up to end with `Code Connect: ✅ mapped`. | | |
