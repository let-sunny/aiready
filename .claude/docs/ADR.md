...
- The per-design section markup in `canicode-gotchas` Step 4 used to pass LLM-rendered prose into `upsert-gotcha-section`, so severity labels and question wording could drift from `gotcha-survey --json`. Fixed by routing survey JSON through `renderGotchaSection` in TypeScript (#439).
