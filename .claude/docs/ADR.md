...
- **Q3 (fallback annotation UX)**: The annotation `labelMarkdown` names the source component (`instanceContext.sourceComponentName` when available, `sourceComponentId` as fallback) and states the recommended target, e.g. *"Apply `itemSpacing: 16` on the source component **Card** to share across all instances. This instance kept its current value to avoid unintended fan-out."* Exact propagation counts ("this would have affected N instances") are a stretch goal — walking every instance of a component in-file is not free in a `use_figma` batch and we do not have measured cost data. Parked for the follow-up; annotations in v1 describe the *target* component, not the *blast radius*.
