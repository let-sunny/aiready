...
${ruleGroups.length === 0
    ...
    <!-- Tab switching + navigation script -->
    <script>
      (function() {
        var container = document.currentScript.parentElement;
        function activate(cat, scrollTo) {
          container.querySelectorAll('.rpt-tab').forEach(function(t) {
            var active = t.dataset.tab === cat;
            t.classList.toggle('active', active);
            t.setAttribute('aria-selected', active);
          });
          container.querySelectorAll('.rpt-tab-panel').forEach(function(p) {
            p.classList.toggle('active', p.dataset.panel === cat);
          });
          container.querySelectorAll('.rpt-gauge-item').forEach(function(g) {
            g.classList.toggle('active', g.dataset.tab === cat);
          });
          if (scrollTo) {
            var tabList = container.querySelector('.rpt-tab-list');
            if (tabList) tabList.scrollIntoView({ behavior: 'smooth', block: 'start' });
          }
        }
        function focusRule(ruleId) {
          var el = container.querySelector('[data-rule="' + ruleId + '"]');
          if (el) {
            el.open = true;
        ...
        container.addEventListener('click', function(e) {
          ...
          if (tab) {
            var isGauge = tab.classList.contains('rpt-gauge-item');
            activate(tab.dataset.tab, isGauge);
            return;
          }
          // Opportunity click → activate category + focus rule
          var opp = e.target.closest('[data-opp-rule]');
          if (opp) {
            var cat = opp.dataset.oppCat;
            var rule = opp.dataset.oppRule;
            if (cat) activate(cat, false);
            setTimeout(function() { focusRule(rule); }, 50);
          }
        });
      })();
    </script>
