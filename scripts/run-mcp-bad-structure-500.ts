...
async function main() {
  console.error("=== MCP Vite bad-structure @500px ===");
  ...
  const vcDir = join(RUN_DIR, "vc-mcp-bad-structure-500");
  ...
  copyFileSync(codePng, join(RUN_DIR, "images/mcp-bad-structure-500-code.png"));
  copyFileSync(join(vcDir, "diff.png"), join(RUN_DIR, "images/mcp-bad-structure-500-diff.png"));

  console.error(`\n=== Comparison @500px ===`);
  console.error(`  MCP good:          92%`);
  console.error(`  MCP bad-structure: ${result.similarity}%`);
  console.error(`  Delta:             ${result.similarity - 92}%p`);
}
